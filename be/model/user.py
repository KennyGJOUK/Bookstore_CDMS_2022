...
class User(mydb_conn.DBConn):
    ...
    def register(self, user_id: str, password: str):
        try:
            ...
            print(user_id, password, 0, token, terminal)
            self.conn.execute(
    ...
    def check_token(self, user_id: str, token: str) -> (int, str):
        cursor = self.conn.execute("SELECT token from usr where user_id=:user_id",
                                        {"user_id": user_id})
        row = cursor.fetchone()
        if row is None:
    ...
    def check_password(self, user_id: str, password: str) -> (int, str):
        cursor = self.conn.execute("SELECT password from usr where user_id=:user_id",
                                        {"user_id": user_id})
        row = cursor.fetchone()
        if row is None:
            return error.error_authorization_fail()
